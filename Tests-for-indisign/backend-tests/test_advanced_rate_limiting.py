...
class TestCooldownIncrement:
    ...
    def test_progressive_cooldown_per_failed_attempt(self, api_session):
        ...
        assert len(cooldowns) >= 2, (
            "Need at least two 429 responses to verify cooldown progression — "
            "rate limit may not have triggered across enough attempt counts"
        )
        for i in range(1, len(cooldowns)):
            assert cooldowns[i] >= cooldowns[i-1], (
                f"Cooldown did not increase: attempt group {11+i} gave {cooldowns[i]}s "
                f"but previous gave {cooldowns[i-1]}s"
            )
