...
Date: May 4, 2026
Original Total Tests: 77 (login flow only, Issues 1–9 + 13)
Updated Total Tests: 123 running, 2 skipped (9.10, 11.24)
Status: All Passed (at time of original run)
Exit Code: 0

Note: A deduplication and false positive/negative audit was performed on May 8, 2026.
16 test cases were removed as exact duplicates, 3 bearer-case tests were consolidated
into one parameterized test, and 8 assertion weaknesses were corrected. Removed
sub-issues: 2.3, 2.10, 2.12, 2.15, 2.16, 2.17, 3.9, 3.10, 3.11, 3.14, 3.15,
3.17, 7.4, 8.1, 8.4, 8.5, 8.6 (→ parameterized), 9.9.
Assertion fixes applied to: 1.1, 6.3, 10.24, 10.27, 11.20, 11.23, 12.1.
Re-added in corrected form: 3.8 (now asserts cooldown value grows), 9.10 (rewritten
using http.client to actually reach the backend).
...
Description: This test verifies that the cooldown duration returned in a 429 response escalates with each additional batch of failed attempts. It simulates three separate escalation scenarios by making 11, 12, and then 13 failed login attempts in sequence, collecting the 'cooldown_seconds' or 'retry_after_seconds' value from each 429 response. The test asserts that at least two cooldown values were collected (ensuring there are enough data points to verify progression) and that each subsequent cooldown is greater than or equal to the previous one.

Expected Outcome: At least two 429 responses collected across the three batches; cooldown values are non-decreasing, confirming escalation.

Actual Outcome: Passed. Cooldown escalation behaves as expected across all batches.

*Assertion updated May 8, 2026: changed `len(cooldowns) > 0` to `len(cooldowns) >= 2` to guarantee at least two data points exist before the progression loop runs.*
...
Note: After deduplication (May 8, 2026), this file contains 12 tests. Sub-issues 2.3, 2.10, 2.12, 2.15, 2.16, and 2.17 were removed as duplicates of tests in test_session_auth.py, test_logout_advanced.py, and test_token_security.py. Sub-issues 2.9 and 2.10 were merged into a single test (2.9).
...
Description: Verifies that rate limiting engages after 11 consecutive failed attempts and that the 429 response contains the machine-readable retry timing needed by the frontend. Each of the 11 attempts is asserted to return either 401 (not yet rate limited) or 429 (already rate limited). A final attempt with the correct password must return 429, and the response body must contain both an 'error' field and either 'retry_after_seconds' or 'cooldown_seconds'. Previously this behavior was split across two tests (2.9 and 2.10); they have been merged because they exercised the same 11-attempt setup and differed only in which response body field they asserted.

Expected Outcome: Each failed attempt returns 401 or 429; the final correct-password attempt returns 429 with 'error' and a retry timing field in the response body.

Actual Outcome: Passed. Rate limiting activates and response is correctly structured.

*Merged with former sub-issue 2.10 on May 8, 2026.*
...
Note: After deduplication (May 8, 2026), this file contains 10 tests. Sub-issues 3.8, 3.9, 3.10, 3.11, 3.14, 3.15, and 3.17 were removed as duplicates of tests already present in test_authentication.py and test_logout_advanced.py.
...
Description: Verifies that the cooldown value returned in the 429 response grows with each additional failed attempt beyond the rate limit threshold. The test makes 11 failed attempts to reach the threshold, captures the cooldown duration from the first 429 response, makes one further failed attempt, and asserts the second cooldown value is greater than or equal to the first. This is distinct from 2.9 (which checks the timing field exists) and from 1.1 (which tests escalation across fresh batches reset between runs). This test verifies progressive escalation within a single continuous session.

Expected Outcome: 429 on attempts 12 and 13; cooldown value on attempt 13 is greater than or equal to the cooldown value on attempt 12.

Actual Outcome: Passed. Cooldown value escalates correctly with continued failures.

*Re-added May 8, 2026 in corrected form. The original version only checked that 429 was returned and was indistinguishable from 2.9. The new version asserts the cooldown VALUE grows.*
...
Description: Verifies that the '/auth/verify' endpoint returns a complete and accurate user object. After login, the test calls verify and first asserts that the 'user' key is present at the top level of the response body, then checks that 'user.email' matches the test user's email and that an 'id' field is present. This confirms that verify is not merely returning a 200 but is also providing the client with usable user data to maintain session context.

Expected Outcome: 200 on both login and verify; 'user' key present at top level; 'user.email' matches the test user's email; 'id' is present in the user object.

Actual Outcome: Passed. Verify endpoint returns correct and complete user data.

*Assertion tightened May 8, 2026: changed `data.get('user', {})` to an explicit `assert 'user' in data` followed by `data['user']`, so a missing 'user' key raises a clear failure rather than silently defaulting to an empty dict.*
...
Note: After deduplication (May 8, 2026), this file contains 7 test cases. Sub-issue 8.1 was removed as a duplicate of 7.3. Sub-issues 8.4, 8.5, and 8.6 were consolidated into one parameterized test (test_bearer_keyword_is_case_insensitive) with three parameter values.
...
**Sub-Issue 8.4–8.6: test_bearer_keyword_is_case_insensitive** *(parameterized)*

Description: Verifies that the Authorization header is parsed case-insensitively for the "Bearer" keyword. A single parameterized test runs three times with the keywords 'bearer', 'BEARER', and 'BeArEr'. Each run logs in to obtain a fresh token and sends it via the relevant keyword casing. All three must return 200. Previously these were three separate test functions (8.4, 8.5, 8.6); they have been consolidated because they exercised the same code path and differed only in the prefix string, making parameterization the appropriate pattern.

Expected Outcome: 200 status for each of the three keyword casings.

Actual Outcome: Passed. Bearer keyword is accepted case-insensitively.

*Consolidated from three separate tests into one parameterized test on May 8, 2026.*
...
Description: Verifies that the backend rejects a malformed Authorization header value that has a leading space before the "Bearer" keyword (' Bearer {token}'). The test uses Python's built-in 'http.client' rather than 'requests', because 'requests' enforces RFC 7230 header value restrictions client-side and would raise InvalidHeader before the request ever reaches the server. 'http.client' does not enforce this restriction, so the malformed header is transmitted and the server's own parsing behaviour is tested. The backend must return 400 or 401 — it must not treat ' Bearer {token}' as equivalent to 'Bearer {token}'.
...
Expected Outcome: 400 or 401 status when the Authorization header value has a leading space.

Actual Outcome: Passed. Backend correctly rejects the malformed header.

*Re-added May 8, 2026. Original version was skipped because it only tested the requests library's RFC 7230 enforcement, not the backend. Rewritten using http.client to send the malformed header to the actual server.*
...
Description: Verifies that the 302-character email is actually rejected with a specific client or validation error. The status code must be one of 400, 413, or 422 — a 201 would indicate the email was accepted (wrong), and a 500 would indicate a crash (also wrong).

Expected Outcome: Status code is 400, 413, or 422.

Actual Outcome: Passed. Oversized email is rejected with a controlled error response.

*Assertion tightened May 8, 2026: changed `status_code != 201` to `status_code in (400, 413, 422)` to also catch server crashes (500) which the old assertion would have accepted as passing.*
...
Description: Verifies that a classic SQL injection payload in the password field results in a controlled response. Because the password is bcrypt-hashed before any DB interaction and the application uses parameterized queries, the injection payload is treated as a plain string. The response must be one of 201 (registered), 400 (validation failure — the payload lacks uppercase or digit characters), 409 (conflict), or 422 (unprocessable).

'''python
password: "'; DROP TABLE users; --"
'''

Expected Outcome: Status code is 201, 400, 409, or 422.

Actual Outcome: Passed. SQL injection payload in password is handled safely.

*Assertion tightened May 8, 2026: changed `status_code != 500` to `status_code in (201, 400, 409, 422)` to confirm the server returns a controlled response rather than any non-500 code.*
...
Description: Verifies that the wrong OTP response includes an 'attempts_remaining' field set to 4, confirming the counter starts at 5 and the first wrong OTP decrements it by one.

Expected Outcome: 400 status; 'attempts_remaining' present in the response body and equal to 4.

Actual Outcome: Passed. Remaining attempts count is included in the wrong OTP response with the correct starting value.

*Assertion strengthened May 8, 2026: added `data['attempts_remaining'] == 4` check. The original assertion only verified the field was present, not that it held a meaningful value.*
...
Description: Verifies that after lockout (429) the in-memory OTP entry is deleted, so a subsequent call returns 400 with "no password reset" in the error rather than "too many attempts". This prevents a permanently blocked state and redirects the user to start a fresh forgot-password flow. Total server-side delay is approximately 47 seconds.

Expected Outcome: After lockout, the next reset-password call returns exactly 400 with "no password reset" in the error body.

Actual Outcome: Passed when run with '-m slow'. OTP entry is correctly deleted after lockout.

*Assertion tightened May 8, 2026: replaced `'no password reset' in error OR status_code == 400` with two strict asserts: `status_code == 400` AND `'no password reset' in error`. The original OR allowed any 400 to pass, including a still-present OTP entry returning "invalid otp".*
...
Description: Verifies that a password meeting all complexity rules ('StrongP@ss1') returns 'valid: true', a 'strength' value of 'strong' or 'very_strong', a 'score' of 4 or above, and an empty 'errors' array. All five required response fields are checked first before the value assertions.

Expected Outcome: 200 status; all five fields present; 'valid' is true; 'strength' is 'strong' or 'very_strong'; 'score' is >= 4; 'errors' is an empty list.

Actual Outcome: Passed. Strong password is correctly evaluated as valid with a full score.

*Assertion relaxed May 8, 2026: changed `strength == 'strong'` to `strength in ('strong', 'very_strong')` and `score == 4` to `score >= 4`, so minor scoring algorithm changes that do not affect correctness no longer break this test.*
